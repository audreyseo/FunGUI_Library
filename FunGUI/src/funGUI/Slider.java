...
//		g.rect(xs, y, 7, 15);
	...
	void slide() {
		...
		if (p.mousePressed && p.mouseX > x - .45 * w && p.mouseX < x + .45 * w
			...
			percent = PApplet.constrain(PApplet.map(( float) (p.mouseX), ( float ) (x - .45 * w), ( float ) (x + .45 * w), 0.0f, 1.0f), 0.0f, 1.0f);
		}
	}

	boolean slid() {
		return (p.mousePressed && p.mouseX > x - .45 * w && p.mouseX < x + .45 * w
				&& p.mouseY > y - 7.5 && p.mouseY < y + 7.5);
	}
